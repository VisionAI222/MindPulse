...
async def telegram_webhook(request: Request):
    ...
    # ⚡ OPTIMIZED WEBHOOK RESPONSE PIPELINE:
    ...
    if analysis.get("risk_level") == "severe":
        ...
        await close_conversation(chat_id)  # Ensure this is awaited if it's an async database call
        
    elif analysis.get("confidence") == "low" and analysis.get("clarifying_question"):
        # If the clarifying question is just the fallback string, let's skip it 
        # and generate a real dynamic AI reply instead!
        ...
        if analysis["clarifying_question"] != fallback_phrase:
            ...
            await save_analysis(chat_id, analysis, analysis["clarifying_question"])
        else:
            # Bypass the static text loop and use the dynamic generator
            ...
            await save_analysis(chat_id, analysis, reply_text)
            
    else:
        # Standard dynamic response path
        try:
            reply_text = await generate_reply(student_text, analysis.get("risk_level", "low"), analysis.get("overall_mood_summary", "Conversing"))
        except Exception as e:
            print(f"[Reply Generation Error]: {e}")
            reply_text = "I'm here for you. Can you tell me a little more about what's on your mind?"
